        ...
        public BasePostData<CommodityInventoryChange> CreateCommodityInventoryChangeInStockChange(BaseData<CommodityCode> baseDataCommodityCode)
        {
			...
			if (null != inList || inList.Count > 0)
            {
				...
				if (null != consumedInList && consumedInList.Count > 0)
				{
					...
					consumedInList.Where(it => it.operate_type == 1).ToList().ForEach(commodityCode =>
					{
						var temp = new CommodityInventoryChange()
						{
							CommodityCodeId = commodityCode.id,
				...
				//未消耗走原逻辑
				var notConsumedInList = inList.Where(it => it.InventoryStatus != CommodityInventoryChangeStatus.已消耗.ToString()).ToList();

				if (null != notConsumedInList && notConsumedInList.Count > 0)
				{
					notConsumedInList.ForEach(commodityCode =>
					{
						var temp = new CommodityInventoryChange()
						{
							CommodityCodeId = commodityCode.id,
